...
/// Derive macro for UniCache when used with omnipaxos and serde.
/// This is just a temp-fix for the `serde` feature. It should be removed once we find a way to check if omnipaxos is compiled with the `serde` feature.
#[proc_macro_derive(UniCacheSerdeEntry, attributes(unicache, snapshot))]
pub fn unicache_serde_entry_derive(input: TokenStream) -> TokenStream {
    // Parse the input tokens into a syntax tree
    let ast = parse_macro_input!(input as DeriveInput);
